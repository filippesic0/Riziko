...
public class AISubmissive implements AI {
    ...
    public String getTrade() {
            ...
            List<Card> cards = cplayer.getCards();
    ...
    public String getPlaceArmies() {
		...
		return getPlaceCommand(randomCountry(cplayer.getTerritoriesOwned()), cplayer.getExtraArmies()/3 + cplayer.getExtraArmies()%3);
    }
    ...
    public String getCapital() {
	    return "capital " + randomCountry(cplayer.getTerritoriesOwned()).getColor();
    }
	...
	protected String getPlaceCommand(Country country, int armies) {
		return "placearmies " + country.getColor() + " " + (!game.getSetupDone()?1:Math.max(1, Math.min(cplayer.getExtraArmies(), armies)));
	}
