	...
	public void needInput(int s) {
		...
		switch (gameState) {
                        case RiskGame.STATE_PLACE_ARMIES:
                            ...
                            break;
			case RiskGame.STATE_PLACE_ARMIES2: {
                                if ( !myrisk.getGame().NoEmptyCountries() ) {
					goButtonText = resb.getString("game.button.go.autoplace");
				}
                                note = getArmiesLeftText();
			...
			/*if ( myrisk.getGame().NoEmptyCountries() && ind==1) {
				ind=0;
				break;
			}
			}*/
            ...
            switch (gameState) {
                ...
                case RiskGame.STATE_FORTIFYING:
                    if (countries.length==0) {
						...
						//Prilikom uvodjenja postojanja vise od jednog premestanja, proverava se uslov
						//da je moguce, sto se tice jedne teritorije, premestati ili u nju, ili iz nje:
						int n=(int)Double.POSITIVE_INFINITY;
						n-=100;
						int from = myrisk.hasArmiesInt(countries[0]);
						if(Risk.inout[countries[0]]>0||Risk.inout[countries[1]]<0)
							//Ako je n>0, nesto smo prebacili iz country2 u country1, pa mozemo da vratimo.
							n=Risk.previousmoves[countries[1]][countries[0]];
						n=Math.min(n,from-1);
						if(n>0)
