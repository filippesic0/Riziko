...
public class BattleDialog extends JDialog implements MouseListener {
	...
	private final static int BUTTON_ROLL= 11;
	private final static int BUTTON_ANNIHILATE= 12;
	private static int ind=0;
	private static int winarmies=0;
	/**
	...
	private void initGUI() {
		...
		GraphicsUtil.setBounds(annihilate, 50, 270, 0, 0);
	...
	public void needInput(int n, boolean c) {
		...
		battle.repaint();
                ...
                if (canRetreat && annihilate.isSelected()) {
                    ...
                    gui.go("roll "+ (canRetreat?noda:nodd) );
                    if(winarmies-1==myrisk.hasArmiesInt(c1num))
                        myrisk.getGame().getCountries()[c1num-1].addArmy();
                }
	}
		...
		public void paintComponent(Graphics g) {
			...
			if (canRetreat) {
				GraphicsUtil.drawImage(g, Battle.getSubimage(481, 105, 98, 40), 336, 265, this); // retreat
                        ...
                        MoveDialog.paintMove(g, c1img, c2img, color1, color2, country1.getName(), country2.getName(),
                        ...
                        GraphicsUtil.drawStringCenteredAt(g, resBundle.getString("battle.roll"), 'R', 240, 289);//196, 270, 88, 31
                        GraphicsUtil.drawStringCenteredAt(g, resBundle.getString("battle.annihilate"), 'A', 94, 289);//196, 270, 88, 31
	...
	public int insideButton(int x, int y) {
		...
		if (GraphicsUtil.insideButton(x, y, 196, 270, w, h)) {//240, 289
			return BUTTON_ROLL;
		}
		if (GraphicsUtil.insideButton(x, y, 50, 270, w, h)) {//240, 289
			return BUTTON_ANNIHILATE;
		}
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (thebutton != 0) {
			...
			switch (thebutton) {
				...
				case BUTTON_ROLL:{
					int noda=Math.min(myrisk.hasArmiesInt(c1num)-1,3);
					int nodd=Math.min(myrisk.hasArmiesInt(c2num),3);

					int[] dicea = new int[noda];

					for (int j=0; j<noda; j++) {
						dicea[j]=r.nextInt( 6 );
					}
					for (int i=0; i<noda-1; i++) {
						int temp, pos=i;

						for(int j=i+1; j<noda; j++)
								if(dicea[j]>dicea[pos])
										pos=j;
						temp = dicea[i];
						dicea[i] = dicea[pos];
						dicea[pos] = temp;
					...
					Country c2=myrisk.getGame().getCountries()[c2num-1];
					Country c1=myrisk.getGame().getCountries()[c1num-1];
					if(dicea[0]>diced[0])
						c2.looseArmy();
					else
						c1.looseArmy();
					...
					{   c2.addArmy();
						ind=1;
						winarmies=myrisk.hasArmiesInt(c1num);
						retreat.setVisible(false);
						annihilate.setVisible(false);
						button.setVisible(false);
						//while(myrisk.hasArmiesInt(c2num)>0){
							annihilate.doClick();
							/*gui.go( "roll " + 3 );
							//gui.go( "roll " + 1 );
							battle.repaint();
